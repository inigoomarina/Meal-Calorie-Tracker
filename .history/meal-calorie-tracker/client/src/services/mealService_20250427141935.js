...
const mealService = {
  ...
  updateMeal: async (id, mealData) => {
    try {
      const response = await axios.put(`/meals/${id}`, mealData);
      return response.data;
    } catch (error) {
  ...
  searchFood: async (query) => {
    try {
      const response = await axios.get(`/meals/search?q=${encodeURIComponent(query)}`);
      return response.data;
    } catch (error) {
  ...
  // Get food details by FDC ID (check if our backend supports this)
  getFoodDetails: async (fdcId) => {
    try {
      const response = await axios.get(`/meals/food/${fdcId}`);
      return response.data;
    } catch (error) {
      console.error('Food details not available on server:', error);
      throw error;
    }
  },
  ...
  getCalorieSummary: async (date) => {
    try {
      ...
      const response = await axios.get('/meals/summary', { 
  ...
  getWeeklyStats: async () => {
    try {
      ...
      const response = await axios.get('/meals/weekly-stats', {
        params: { _t: timestamp },
        headers: {
          'Cache-Control': 'no-cache',
          'Pragma': 'no-cache'
