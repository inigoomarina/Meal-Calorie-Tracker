...
const mealService = {
  ...
  createMeal: async (mealData) => {
    try {
      ...
      const formattedMealData = {
        ...
        nutrition: {
          protein: parseFloat(mealData.nutrition.protein),
          carbs: parseFloat(mealData.nutrition.carbs),
          fat: parseFloat(mealData.nutrition.fat),
          fiber: parseFloat(mealData.nutrition.fiber || 0),
          sugar: parseFloat(mealData.nutrition.sugar || 0)
        }
      };
  ...
  getUserMeals: async (filter = {}) => {
    try {
      ...
      // If meals aren't in an array format, handle that case
      if (!Array.isArray(response.data)) {
        console.warn('Expected meals data to be an array, got:', typeof response.data);
        return [];
      }
