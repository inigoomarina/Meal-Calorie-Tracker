...
const Dashboard = () => {
  ...
  useEffect(() => {
    const loadDashboardData = async () => {
      ...
      try {
        ...
        console.log("Today's meals raw data structure:", 
          meals && meals.length ? meals[0] : "No meals found");
        
        // Check if nutrition data exists in the response and how it's structured
        if (meals && meals.length > 0) {
          console.log("Sample meal nutrition:", meals[0].nutrition);
          console.log("Sample meal time:", meals[0].time);
        }
  ...
  const calculateMacros = () => {
    try {
      ...
      todaysMeals.forEach(meal => {
        try {
          // Safely extract and add each nutrient value
          protein += parseFloat(meal.nutrition?.protein) || 0;
          carbs += parseFloat(meal.nutrition?.carbs) || 0;
          fat += parseFloat(meal.nutrition?.fat) || 0;
        } catch (err) {
  ...
  // Use the safer macro calculation
