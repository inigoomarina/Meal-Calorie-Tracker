...
const Dashboard = () => {
  ...
  useEffect(() => {
    ...
    const loadDashboardData = async () => {
      ...
      try {
        ...
        await refreshUser(); // Use refreshUser from context
        // Re-check mount status after await
        if (!isMounted) {
          console.log("Dashboard: Aborted after refreshUser (unmounted).");
          return; 
        }
        ...
        try {
          ...
          if (!isMounted) {
             console.log("Dashboard: Aborted after fetching meals (unmounted).");
             return;
          }
        ...
        // Process meals (ensure isMounted checks if necessary inside loops/maps if they are async)
        ...
        if (!isMounted) {
          console.log("Dashboard: Aborted after processing meals (unmounted).");
          return;
        }
        ...
        try {
          ...
          if (!isMounted) {
            console.log("Dashboard: Aborted after fetching weekly stats (unmounted).");
            return;
          }
    ...
    if (urlSearchParams.has('refresh')) {
      ...
      // The refreshCounter state update might be redundant if loadDashboardData runs anyway
      // setRefreshCounter(prev => prev + 1); // Consider if this is needed
    }
  ...
  // Use refreshUser directly from context, ensure it's stable
  }, [user, refreshCounter, location.search, refreshUser]); 
  ...
  const handleGoalSettingsSave = useCallback(async (newGoals) => {
    ...
    try {
      ...
      // No need to manually update local state if 'user' effect handles it
      // Or update optimistically if preferred:
  ...
  if (isLoading && todaysMeals.length === 0) { // Improved loading condition
  ...
  return (
    <div className="dashboard">
      ...
      <GoalSettings 
        ...
        initialGoals={{
          calorieGoal: user?.calorieGoal ?? 2000, // Use nullish coalescing for defaults
