...
const LogMeal = () => {
  ...
  // Helper function to calculate full nutrition data from the selected food and portion
  const calculateNutrients = () => {
    if (!selectedFood) return null;
    
    // Calculate the multiplier based on grams
    const multiplier = grams / 100;
    console.log("Calculating nutrients with multiplier:", multiplier);
    
    // Get nutrition data with detailed macronutrients
    const nutrients = {
      calories: Math.round((selectedFood.calories || 0) * multiplier),
      protein: parseFloat(((selectedFood.protein || 0) * multiplier).toFixed(1)),
      carbs: parseFloat(((selectedFood.carbs || 0) * multiplier).toFixed(1)),
      fat: parseFloat(((selectedFood.fat || 0) * multiplier).toFixed(1)),
      fiber: parseFloat(((selectedFood.fiber || 0) * multiplier).toFixed(1)),
      sugar: parseFloat(((selectedFood.sugar || 0) * multiplier).toFixed(1)),
    };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Make sure nutrition data is stored in both places for backward compatibility
      const mealData = {
        name: selectedFood.name,
        portion: grams / 100, // Store portion as a multiplier of 100g
        calories: nutrients.calories,
        // Store as both direct properties and in nutrition object for API flexibility
        protein: nutrients.protein,
        carbs: nutrients.carbs,
        fat: nutrients.fat,
        fiber: nutrients.fiber,
        sugar: nutrients.sugar,
        // Also include a structured nutrition object
        nutrition: {
          protein: nutrients.protein,
          carbs: nutrients.carbs,
          fat: nutrients.fat,
