...
const MealHistory = () => {
  ...
  const renderCalendarView = () => {
    ...
    const dates = Object.keys(dailyTotals).sort((a, b) => 
      sortOrder === 'newest' ? new Date(b) - new Date(a) : new Date(a) - new Date(b)
    );
    ...
    return (
      <div className="calendar-view">
        {dates.map(date => {
          ...
          const isOverLimit = dayTotal.calories > calorieGoal;
            ...
            <div key={date} className="calendar-day">
              ...
              <div className={`calendar-totals ${isOverLimit ? 'over-limit' : 'under-limit'}`}>
                <div className="total-item">
                  <span>Calories:</span> {Math.round(dayTotal.calories)}
                  <span className="goal-indicator">
                    {isOverLimit 
                      ? `(${Math.round(dayTotal.calories - calorieGoal)} over goal)` 
                      : `(${Math.round(calorieGoal - dayTotal.calories)} under goal)`}
                  </span>
                </div>
                <div className="total-item"><span>Protein:</span> {dayTotal.protein.toFixed(1)}g</div>
                <div className="total-item"><span>Carbs:</span> {dayTotal.carbs.toFixed(1)}g</div>
                <div className="total-item"><span>Fat:</span> {dayTotal.fat.toFixed(1)}g</div>
              </div>
              <div className="calendar-meals">
                <h4>Meals</h4>
                <ul>
                  {dayTotal.meals.map(meal => (
