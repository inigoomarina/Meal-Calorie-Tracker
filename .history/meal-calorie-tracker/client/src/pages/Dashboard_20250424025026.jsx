...
const Dashboard = () => {
  ...
  // Enhanced macro calculation function with better debugging and validation
  ...
  const extractNutritionValue = (meal, nutrient) => {
    // Check for direct property first (legacy format)
    if (typeof meal[nutrient] === 'number' || typeof meal[nutrient] === 'string') {
      const numValue = parseFloat(meal[nutrient]);
      if (!isNaN(numValue)) return numValue;
    }
    
    // Check in nutrition object (preferred format)
    if (meal.nutrition && (typeof meal.nutrition[nutrient] === 'number' || typeof meal.nutrition[nutrient] === 'string')) {
      const numValue = parseFloat(meal.nutrition[nutrient]);
      if (!isNaN(numValue)) return numValue;
    }
