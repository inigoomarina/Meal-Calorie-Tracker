...
const Dashboard = () => {
  ...
  // macroTargets holds the GOALS
  const [macroTargets, setMacroTargets] = useState({ carbs: 150, protein: 100, fat: 70 });
  // consumedMacros holds the calculated CONSUMED values for TODAY
  ...
  useEffect(() => {
    if (user) {
      ...
      setCalorieData(prev => ({
        ...
        remaining: userCalorieGoal - prev.consumed // Recalculate remaining based on new goal
      }));
      // Set the GOALS
      setMacroTargets({
  ...
  useEffect(() => {
    ...
    const loadDashboardData = async () => {
      ...
      try {
        ...
        // Sort meals by time (e.g., most recent first)
        ...
        if (isMounted) {
          ...
          // Calculate and set today's consumed macros
  ...
  // Accepts meals array as argument to ensure it uses the correct data
  const calculateMacros = (mealsToCalculate) => {
    return mealsToCalculate.reduce((totals, meal) => {
  ...
  const extractNutritionValue = (meal, nutrient) => {
    // Ensure nutrition object and the specific nutrient exist and are numbers
    const value = meal?.nutrition?.[nutrient];
    ...
    <div className="dashboard">
      ...
      <div className="card macros-section">
        ...
        <div className="calories-display">
          <CircularProgress
            value={calorieData.consumed} // Today's consumed calories
            max={calorieData.goal} // Today's calorie goal
            color="#FF9800"
      ...
      {/* Macros section - Display CONSUMED vs GOAL */}
      <div className="card macros-section">
        <h2>Macros</h2>
        <div className="macros-gauges">
          <CircularProgress
            value={consumedMacros.carbs} // Today's consumed carbs
            max={macroTargets.carbs} // Carb goal
            color="#4CAF50"
            title="Carbs"
            unit="g"
            subtitle={`Goal: ${macroTargets.carbs}g`} // Show goal in subtitle
          />
          <CircularProgress
            value={consumedMacros.fat} // Today's consumed fat
            max={macroTargets.fat} // Fat goal
            color="#9C27B0"
            title="Fat"
            unit="g"
            subtitle={`Goal: ${macroTargets.fat}g`} // Show goal in subtitle
          />
          <CircularProgress
            value={consumedMacros.protein} // Today's consumed protein
            max={macroTargets.protein} // Protein goal
            color="#2196F3"
            title="Protein"
            unit="g"
            subtitle={`Goal: ${macroTargets.protein}g`} // Show goal in subtitle
          />
        </div>
      </div>

      {/* Today's Meals - Updated Grid Display */}
          ...
          <div className="meals-grid">
            {todaysMeals.map(meal => (
              <div key={meal._id} className="meal-card" onClick={() => handleMealClick(meal)}>
                ...
                <div className="meal-calories">{Math.round(meal.calories || 0)} calories</div>
                <div className="meal-macros-summary">
                  <span>P: {Math.round(extractNutritionValue(meal, 'protein'))}g</span>
                  <span>C: {Math.round(extractNutritionValue(meal, 'carbs'))}g</span>
                  <span>F: {Math.round(extractNutritionValue(meal, 'fat'))}g</span>
                </div>
                <div className="meal-time">
                  {formatMealTime(meal.time)} {/* Use only time format here */}
                </div>
              </div>
            ))}
          </div>
