...
const Dashboard = () => {
  ...
  useEffect(() => {
    const loadDashboardData = async () => {
      ...
      try {
        ...
        console.log("Today's meals received:", meals);
        
        // Sort meals by time (newest first) and ensure they have proper time values
        const mealArray = Array.isArray(meals) ? meals : [];
        const sortedMeals = [...mealArray]
          .map(meal => ({
            ...meal,
            // Make sure time is in proper format if it's missing
  ...
  // Calculate current macros from today's meals more carefully with better error logging
  const currentMacros = {
    protein: todaysMeals.reduce((sum, meal) => {
      try {
        // Check if nutrition exists
        if (!meal.nutrition) {
          console.warn(`Meal ${meal.name} is missing nutrition data`);
          return sum;
        }
