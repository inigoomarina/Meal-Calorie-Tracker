...
const Dashboard = () => {
  ...
  const extractNutritionValue = (meal, nutrient) => {
    ...
    // First check direct property on the meal object (backward compatibility)
    if (typeof meal[nutrient] !== 'undefined' && meal[nutrient] !== null) {
      const directValue = parseFloat(meal[nutrient]);
      if (!isNaN(directValue)) return directValue;
    }
    
    // Then check in the nutrition object (preferred location)
    if (meal.nutrition && typeof meal.nutrition[nutrient] !== 'undefined' && meal.nutrition[nutrient] !== null) {
      const nestedValue = parseFloat(meal.nutrition[nutrient]);
      if (!isNaN(nestedValue)) return nestedValue;
    }
  ...
  const calculateMacros = () => {
    if (!todaysMeals || todaysMeals.length === 0) {
      console.log("No meals to calculate macros from");
      return { protein: 0, carbs: 0, fat: 0 };
    }
    
    console.log("Calculating macros from", todaysMeals.length, "meals");
    ...
    todaysMeals.forEach((meal, index) => {
      try {
        ...
        console.log(`Meal ${index + 1} (${meal.name}) nutrition:`, {
          protein: mealProtein,
          carbs: mealCarbs,
          fat: mealFat
        });
    ...
    console.log("Total macros calculated:", result);
    return result;
  };
  
  // Enhanced meal card display to clearly show macronutrients
  const renderMealCard = (meal) => {
    // Extract nutrition values
    const protein = extractNutritionValue(meal, 'protein');
    const carbs = extractNutritionValue(meal, 'carbs');
    const fat = extractNutritionValue(meal, 'fat');
      ...
      >
        ...
        <div className="meal-macros">
          <div className="macro-item">
            ...
            <span className="macro-value">{protein.toFixed(1)}g</span>
          </div>
          <div className="macro-item">
            <span className="macro-label">C:</span>
            <span className="macro-value">{carbs.toFixed(1)}g</span>
          </div>
          <div className="macro-item">
            <span className="macro-label">F:</span>
            <span className="macro-value">{fat.toFixed(1)}g</span>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="dashboard">
      ...
      {/* Enhanced Meal Detail Modal with clearer macro display */}
      {selectedMeal && (
        <MealDetailModal 
          meal={{
            ...selectedMeal,
            ...
            nutrition: {
              protein: extractNutritionValue(selectedMeal, 'protein'),
              carbs: extractNutritionValue(selectedMeal, 'carbs'),
              fat: extractNutritionValue(selectedMeal, 'fat'),
              fiber: extractNutritionValue(selectedMeal, 'fiber'),
              sugar: extractNutritionValue(selectedMeal, 'sugar')
            }
          }} 
