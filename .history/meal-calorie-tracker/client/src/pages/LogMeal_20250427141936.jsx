...
const LogMeal = () => {
  ...
  const debouncedSearch = useCallback(
    debounce(async (query) => {
      ...
      try {
        // Try using our server's endpoint first (more reliable)
        ...
        if (results && Array.isArray(results) && results.length > 0) {
          ...
          setSearchResults([]);
          setError('No foods found matching your search. Try another term.');
        }
      } catch (err) {
        console.error('Food search error:', err);
        setError('Food search is temporarily unavailable. Please try again later.');
      } finally {
        setIsSearching(false);
      }
    }, 500),
  ...
  const handleFoodSelect = async (food) => {
    ...
    try {
      ...
      if (detailedFood) {
        ...
        throw new Error("Could not retrieve detailed food information");
      }
    } catch (err) {
      console.error('Error getting food details:', err);
      // Create a minimal food object with the data we already have
      setSelectedFood({
        fdcId: food.fdcId,
      ...
      setError('Some nutrition details may be incomplete, but you can still log this item.');
    } finally {
      setIsSearching(false);
    }
  };
