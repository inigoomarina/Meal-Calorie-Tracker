    ...
    const loadDashboardData = async () => {
      ...
      try {
        ...
        try {
          ...
          // Ensure we only get meals for today by using the specific date parameter
        ...
        const processedMeals = mealArray
          .map((meal, index) => {
            try {
              // Stricter filtering by date to ensure only today's meals
        ...
        // Calculate today's calories directly from processed meals to ensure accuracy
        const todayTotalCalories = processedMeals.reduce((sum, meal) => sum + (meal.calories || 0), 0);
        
        // Update calorieData with proper goal and today's calories only
