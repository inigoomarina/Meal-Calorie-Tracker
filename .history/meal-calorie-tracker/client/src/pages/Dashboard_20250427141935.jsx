    ...
    const loadDashboardData = async () => {
      ...
      try {
        ...
        try {
          ...
          // Force current date for today's meals
        ...
        // Filter meals to only include ones from today
        const processedMeals = mealArray
          .map((meal, index) => {
            try {
              ...
              // Compare dates (not timestamps) to filter by today only
              const mealDateOnly = new Date(mealDate);
              mealDateOnly.setHours(0, 0, 0, 0);
              
              // Skip meals that are not from today
              if (mealDateOnly.getTime() !== currentDate.getTime()) {
                console.log(`Skipping meal from ${mealDate.toDateString()}, not today (${currentDate.toDateString()}):`, meal.name);
                return null;
              }
        ...
        console.log("Today's processed meals:", processedMeals);
        ...
        // Calculate today's calories directly from the filtered meals
        ...
        setError(`Failed to load dashboard data: ${err.message}. Please try refreshing the page.`);
      } finally {
        setIsLoading(false);
      }
    };
