...
const LogMeal = () => {
  ...
  const handleFoodSelect = async (food) => {
    ...
    try {
      ...
      console.log("Retrieved food details:", detailedFood);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!nutrients) {
        // This case should ideally not happen if selectedFood is valid, but good to check
        throw new Error("Failed to calculate nutrients. Please re-select the food.");
      }
      ...
      // Log the specific error and provide user feedback
      console.error('Failed to log meal:', err);
      const errorMessage = err.response?.data?.message || err.message || 'Failed to log meal. Please try again later.';
      setError(errorMessage);
      setIsSubmitting(false); // Ensure button is re-enabled on error
    } 
    // No finally block needed for setIsSubmitting(false) here, as navigation replaces the component
  };
  ...
  return (
    <div className="log-meal-container">
      ...
      {isSearching && !selectedFood && ( // Show loading only when fetching details
        ...
        <div className="meal-details-section">
          ...
          <form onSubmit={handleSubmit}>
            ...
            <button 
              ...
              disabled={isSubmitting || isSearching} // Disable if submitting or fetching details
            >
