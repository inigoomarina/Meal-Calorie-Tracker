...
const Dashboard = () => {
  ...
  const [consumedMacros, setConsumedMacros] = useState({ protein: 0, carbs: 0, fat: 0 }); // State for today's consumed macros
  ...
  useEffect(() => {
    ...
    const loadDashboardData = async () => {
      ...
      try {
        ...
        // Sort meals by time ascending for today's view
        const sortedMeals = [...mealArray].sort((a, b) => new Date(a.time) - new Date(b.time));
        ...
        const todayConsumedMacros = sortedMeals.reduce((totals, meal) => {
        ...
        if (isMounted) {
          ...
          // Update consumed macros state
          setConsumedMacros({
            protein: Math.round(todayConsumedMacros.protein),
            carbs: Math.round(todayConsumedMacros.carbs),
            fat: Math.round(todayConsumedMacros.fat)
          });
        }
  ...
  const extractNutritionValue = (meal, nutrient) => {
    // Ensure nutrition object and property exist and are numbers
  ...
  const handleMealClick = (meal) => {
    // Prepare meal data for modal, ensuring values are numbers
    const modalMealData = {
      ...meal,
      formattedTime: formatMealDateTime(meal.time),
      calories: meal.calories || 0,
      nutrition: {
        protein: extractNutritionValue(meal, 'protein').toFixed(1),
        carbs: extractNutritionValue(meal, 'carbs').toFixed(1),
        fat: extractNutritionValue(meal, 'fat').toFixed(1),
        // Add others if needed in modal
      }
    };
    setSelectedMeal(modalMealData);
  };
  ...
  return (
    <div className="dashboard">
      ...
      {/* Meal Detail Modal - UPDATED props */}
      {selectedMeal && (
        <MealDetailModal
          meal={selectedMeal} // Pass the prepared modal data
