...
const Dashboard = () => {
  ...
  useEffect(() => {
    const loadDashboardData = async () => {
      ...
      try {
        ...
        // Process and normalize meal data - Ensure calories, protein, carbs, fat are parsed correctly
        ...
        const processedMeals = mealArray.map(meal => {
          ...
          const nutritionData = {
            // Check both direct properties and nutrition object, parse to float
          ...
          // Ensure calories are parsed as integer
          const mealCalories = parseInt(meal.calories) || 0;

          console.log(`Processed nutrition for meal "${meal.name}": Calories: ${mealCalories}, Nutrition:`, nutritionData);
          
          // Create a properly formatted meal object
          return {
            ...meal,
            time: meal.time || meal.createdAt || new Date().toISOString(),
            calories: mealCalories, // Use parsed calories
        ...
        setTodaysMeals(sortedMeals); // Update state with processed and sorted meals
        
        // Load calorie summary for today (should reflect sum of meal calories)
        ...
        // Set calorie data based on summary API response
        setCalorieData({
          consumed: summaryData.totalCalories || 0, // Use summary total
          goal: user?.calorieGoal || 2000,
          remaining: (user?.calorieGoal || 2000) - (summaryData.totalCalories || 0)
        });

        // Load weekly stats for charts (ensure API provides aggregated macros if needed)
        console.log("Fetching weekly stats...");
        const weekStats = await mealService.getWeeklyStats();
        console.log("Weekly stats:", weekStats);
        
        // Set weekly data (used by WeeklyCaloriesDisplay)
        setWeeklyData({
          labels: weekStats?.days || [],
          calories: weekStats?.calories || [],
          proteins: weekStats?.proteins || [], // Ensure API provides these if needed elsewhere
          carbs: weekStats?.carbs || [],     // Ensure API provides these if needed elsewhere
          fats: weekStats?.fats || []         // Ensure API provides these if needed elsewhere
        });
      } catch (err) {
  ...
  // Enhanced macro calculation function - This sums macros from the processed `todaysMeals` state
  ...
  // Helper function to extract nutrition values safely - This handles parsing
  ...
  // Use the enhanced calculation for macro progress gauges
  const currentMacros = calculateMacros();
  console.log("Current macros calculated for display:", currentMacros);
      ...
      {selectedMeal && (
        <MealDetailModal 
          meal={{
            ...
            // Ensure modal gets correctly parsed/formatted data
            ...
            nutrition: {
              ...
              // Include fiber/sugar if available and needed in modal
