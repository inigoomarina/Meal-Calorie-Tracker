...
const MealCalendar = ({ meals, onDeleteMeal }) => {
  ...
  const getMealsByDate = () => {
    ...
    meals.forEach(meal => {
      try {
        ...
        // This prevents timezone shifts from moving meals across day boundaries.
  ...
  const formatDate = (dateString) => {
    // Create date object assuming the string is YYYY-MM-DD in UTC
    const date = new Date(dateString + 'T00:00:00Z'); // Add Z to specify UTC
    const options = { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric', timeZone: 'UTC' }; // Display in UTC context
